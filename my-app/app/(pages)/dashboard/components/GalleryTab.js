...
const GalleryTab = () => {
  ...
  const handleOrderChange = async (id, newOrder) => {
    ...
    const updatedImages = images
      .map(img => (img._id === id ? { ...img, order: parseInt(newOrder) || 0 } : img))
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  {selectedFile ? (
                    <div className="text-center">
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        className="h-8 w-8 text-green-500 mb-2 mx-auto"
                        viewBox="0 0 20 20"
                        fill="currentColor"
                      >
                        <path
                    ...
                    <>
                      <svg
                        className="w-8 h-8 mb-3 text-gray-400 group-hover:text-green-500 transition-colors"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
                        viewBox="0 0 20 16"
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Classify this image to help categorize the gallery.
                </p>
              </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {images.map(img => (
              <div
                key={img._id}
                className="relative group aspect-square rounded-2xl overflow-hidden border border-gray-100 bg-gray-50 shadow-sm hover:shadow-md transition-all duration-300 transform"
              >
                <img
                  src={img.imageLink}
                  alt={img.domain}
                ...
                <div className="absolute top-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="bg-white/90 backdrop-blur-sm rounded-lg p-1 shadow-lg border border-gray-100">
                    <span className="text-[10px] text-gray-500 uppercase font-bold px-1 block mb-0.5">
                      Order
                    </span>
                    <input
                      type="number"
