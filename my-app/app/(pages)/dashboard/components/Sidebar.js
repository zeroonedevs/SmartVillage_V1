...
const Sidebar = ({
  ...
  return (
    <div
      className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-white border-r border-gray-200 transition-all duration-300 flex flex-col fixed h-screen z-40 shadow-sm`}
    >
      {/* Sidebar Header */}
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar">
        {tabs.map(tab => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-md transition-all duration-200 group ${
              activeTab === tab.id
          ...
          >
            ...
            {sidebarOpen && <span className="text-sm tracking-wide">{tab.label}</span>}
          </button>
        ))}
      </nav>
