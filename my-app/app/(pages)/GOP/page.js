...
const GOPRegistration = () => {
  ...
  const filteredCountries = countries.filter(
    c => c.name.toLowerCase().includes(searchTerm.toLowerCase()) || c.dial_code.includes(searchTerm)
  );
  ...
  const validateStep = step => {
    switch (step) {
      ...
      case 3:
        return formData.tenure && formData.interestedDomains.length > 0;
      default:
        return true;
    }
  };
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-[Poppins]">
        <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 text-center animate-fade-in-up">
          ...
          <p className="text-gray-500 mb-8">
            Thank you for joining the Global Outreach Program. Our team will review your application
            and contact you shortly.
          </p>
          <Link
            href="/"
            className="inline-flex items-center justify-center px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors w-full"
          >
            Return to Home
          </Link>
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full p-8 md:p-12 justify-between">
          <div>
            ...
            <p className="text-lg text-green-100/90 leading-relaxed font-light">
              Join a network of visionary organizations transforming rural landscapes through
          ...
          <div className="pt-8 border-t border-white/10 hidden md:block">
            ...
            <a
              href="mailto:saivijay.ceo@gmail.com"
            ...
            <div className="flex justify-between items-center relative">
              ...
              {steps.map(step => (
                <div key={step.id} className="flex flex-col items-center bg-white px-2">
                  ...
                  <span
                    className={`text-[10px] md:text-xs font-bold uppercase mt-2 tracking-wide ${
                    ...
                    <div className="flex bg-gray-50 border border-gray-200 rounded-lg focus-within:ring-4 focus-within:ring-green-500/10 focus-within:border-[#008000] focus-within:bg-white transition-all">
                      <div className="relative border-r border-gray-200" ref={dropdownRef}>
                        <button
                          ...
                          className="h-full px-3 flex items-center gap-2 hover:bg-gray-100 rounded-l-lg transition-colors"
                        >
                          ...
                          <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-200 shadow-xl rounded-lg z-50 max-h-60 overflow-y-auto">
                            <div className="p-2 sticky top-0 bg-white border-b z-10">
                              <input
                                ...
                                className="w-full p-2 text-sm border rounded bg-gray-50 focus:outline-none focus:border-[#008000]"
                                autoFocus
                              />
                            </div>
                            {filteredCountries.map(c => (
                              <div
                                ...
                                className="px-4 py-2 hover:bg-green-50 cursor-pointer flex items-center gap-3 text-sm"
                              >
                                <span>{c.flag}</span>{' '}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {['3 Days', '5 Days', '7 Days', '10 Days'].map(days => (
                      <div
                        key={days}
                        onClick={() => setFormData(p => ({ ...p, tenure: days }))}
                        className={`cursor-pointer border rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition-all ${formData.tenure === days ? 'bg-green-50 border-[#008000] ring-1 ring-[#008000] shadow-sm' : 'bg-white border-gray-200 hover:border-[#008000]/50 hover:bg-gray-50'}`}
                      >
                        <Calendar
                          className={`w-5 h-5 ${formData.tenure === days ? 'text-[#008000]' : 'text-gray-400'}`}
                        />
                        <span
                          className={`font-bold text-sm ${formData.tenure === days ? 'text-[#008000]' : 'text-gray-700'}`}
                        >
                          {days}
                        </span>
                      </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {domains.map(domain => (
                      <div
                        ...
                        className={`cursor-pointer p-3.5 rounded-xl border flex items-center justify-between transition-all group ${formData.interestedDomains.includes(domain) ? 'bg-gray-900 border-gray-900 text-white shadow-md' : 'bg-white border-gray-200 text-gray-700 hover:border-gray-300 hover:bg-gray-50'}`}
                      >
                ...
                <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 space-y-6">
                  <div className="flex justify-between items-start border-b border-gray-200 pb-4">
                    ...
                    <button
                      ...
                      className="text-xs text-[#008000] font-bold hover:underline"
                    >
                      EDIT
                    </button>
                  </div>
            ...
            <div className="pt-8 flex items-center justify-between border-t border-gray-100 mt-8">
              {currentStep > 1 ? (
                <button
                  type="button"
                  onClick={prevStep}
                  className="px-6 py-3 rounded-lg font-bold text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors flex items-center gap-2"
                >
