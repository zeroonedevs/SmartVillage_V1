...
const seedData = async () => {
  try {
    ...
    for (const hero of heroes) {
      await Hero.findOneAndUpdate({ title: hero.title }, hero, { upsert: true, new: true });
      console.log(`Processed Hero: ${hero.title}`);
    }

    // Insert Areas of Work
    for (const area of areas) {
      await AreaOfWork.findOneAndUpdate({ title: area.title }, area, { upsert: true, new: true });
      console.log(`Processed Area: ${area.title}`);
    }

    // Insert Focus Areas
    for (const focus of focusAreas) {
      await FocusArea.findOneAndUpdate({ title: focus.title }, focus, { upsert: true, new: true });
